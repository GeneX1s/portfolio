...
<div class="col-lg-8">
  ...
  <form method="post" action="/dashboard/ryr/schedules/{{$schedule->id}}/participant" class="mb-5"
    ...
    <div id="participants-container-2">
      ...
      @foreach ($preselectedParticipants as $index => $participant)
      <div class="mb-3" id="participant-{{ $index + 1 }}">
        <label for="id_member_{{ $index + 1 }}" class="form-label">Murid {{ $index + 1 }}</label>
        <select class="form-control" id="id_member_{{ $index + 1 }}" name="participants[{{ $index + 1 }}][id_member]"
          required>
          @foreach ($members as $member)
          <option value="{{ $member->id }}" {{ $member->id == $participant->id_member ? 'selected' : '' }}>
            {{ $member->nama_murid }}
          </option>
          @endforeach
        </select>
        <button type="button" class="btn btn-danger mt-2"
          onclick="removeParticipant('participant-{{ $index + 1 }}')">-</button>
      </div>
      @endforeach
    </div>
